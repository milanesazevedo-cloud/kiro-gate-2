...
from kiro.auth_multi import TokenInfo, AuthType, MultiTokenAuthManager  # noqa: F401
...
class KiroAuthManager:
    ...
    def is_token_fresh_for_streaming(self, min_validity_seconds: float = 600) -> bool:
        """
        Checks if the token is fresh enough for a long streaming request.

        This ensures the token won't expire during a streaming request that
        might take longer than the normal token validity.
    ...
    async def _background_token_refresh(self) -> None:
        """
        Background task that proactively refreshes the token before expiry.
        ...
        while True:
            try:
                ...
                # the token to expire before we acquired the lock
                async with self._lock:
                    if self._shutdown:
                        break
